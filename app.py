...
def get_gdrive_client():
    gdrive_credentials = st.secrets.get("gdrive")
    if not gdrive_credentials:
        raise RuntimeError("Credenziali Google Drive mancanti in st.secrets['gdrive']")
    scope = [
        ...
        if not docenti_assenti:
            ...
            if ore_assenti.empty:
                ...
                for _, row in ore_assenti.iterrows():
                    ...
                    docenti_disponibili = presenti_ora[(~presenti_ora["Docente"].isin(docenti_assenti))]
                    proposto = "Nessuno"
                ...
                for idx, riga in sostituzioni_df.iterrows():
                    presenti_ora = orario_df[(orario_df["Giorno"] == giorno_assente) & (orario_df["Ora"] == riga["Ora"])]
                    presenti_docenti = presenti_ora["Docente"].unique()
                    sostegni = orario_df[orario_df["Tipo"] == "Sostegno"]["Docente"].unique()
                    opzioni_validi = []
                    for d in presenti_docenti:
                    ...
                    docenti_esclusi_per_ora = orario_df[(orario_df["Giorno"] == giorno_assente) & (orario_df["Ora"] == riga["Ora"]) & (orario_df["Escludi"])]["Docente"].unique()
                    opzioni_esclusi = [d for d in docenti_esclusi_per_ora if d not in docenti_assenti]
                    opzioni = ["Nessuno"] + sorted(opzioni_validi, key=lambda x: (0 if str(x).startswith("[S]") else 1, x)) + sorted(opzioni_esclusi)
